...
{
  m_antenna = antenna;
  m_state->SetupAntennaListener (m_antenna);
  m_interference.SetupAntennaListener (m_antenna);
}
...
YansWifiPhy::StartReceivePacket (Ptr<Packet> packet,
                                 double rxPowerDbm[WifiAntennaModel::NUMBER_OF_ANTENNA_MODES],
                                 ...
                                 enum WifiPreamble preamble)
{
  ...
  for (int i = 0; i < WifiAntennaModel::NUMBER_OF_ANTENNA_MODES; i++){
    rxPowerW [i] = DbmToW (rxPowerDbm[i] + m_rxGainDb);
  }

  for(int k = 0; k < WifiAntennaModel::NUMBER_OF_ANTENNA_MODES; k++){
    NS_LOG_DEBUG("rxPower=" << rxPowerDbm[k] << "dbm");
  }
  ...
  switch (m_state->GetState ())
    ...
    case YansWifiPhy::RX:
      NS_LOG_DEBUG ("drop packet because already in Rx (power=" <<
                    rxPowerW[m_antenna->GetAntennaMode ()] << "W)");
      NotifyRxDrop (packet);
      if (endRx > Simulator::Now () + m_state->GetDelayUntilIdle ())
        {
    ...
    case YansWifiPhy::CCA_BUSY:
    case YansWifiPhy::IDLE:
      if (rxPowerW[m_antenna->GetAntennaMode ()] > m_edThresholdW)
        {
          if (IsModeSupported (txMode) || IsMcsSupported(txMode))
            {
              NS_LOG_DEBUG ("sync to signal (power=" << rxPowerW[m_antenna->GetAntennaMode ()] <<
                            "W), antenna mode = " << m_antenna->GetAntennaMode ());
              // sync to signal
      ...
      else
        {
          NS_LOG_DEBUG ("drop packet because signal power too Small (" <<
                        rxPowerW[m_antenna->GetAntennaMode ()] << "<" << m_edThresholdW << ")");
          NotifyRxDrop (packet);
          goto maybeCcaBusy;
        }
      break;
...
maybeCcaBusy:
  ...
  int tmpAntennaMode = m_antenna->GetAntennaMode ();
  for(int i = 0; i < WifiAntennaModel::NUMBER_OF_ANTENNA_MODES; i++){
    m_antenna->SetAntennaMode (i);
  ...
  m_antenna->SetAntennaMode (tmpAntennaMode);
}
  ...
  if (m_random->GetValue () > snrPer.per)
    {
      ...
      double signalDbm = RatioToDb (event->GetRxPowerW (m_antenna->GetAntennaMode ())) + 30;
      double noiseDbm = RatioToDb (event->GetRxPowerW (m_antenna->GetAntennaMode ()) / snrPer.snr) - GetRxNoiseFigure () + 30;
