...
public:
   ...
   */
  virtual void NavStart (Time duration, int mode) = 0;
  /**
...
private:
  ...
  void NotifyNav (Ptr<const Packet> packet,const WifiMacHeader &hdr, WifiMode txMode, WifiPreamble preamble, int mode);
  /**
  ...
  EventId m_sendAckEndEvent;            //!< ACK transmission completion event
  ...
  Time m_lastNavStart[WifiAntennaModel::NUMBER_OF_ANTENNA_MODES];     //!< The time when the latest NAV started
  Time m_lastNavDuration[WifiAntennaModel::NUMBER_OF_ANTENNA_MODES];  //!< The duration of the latest NAV
  int m_antennaMode;
