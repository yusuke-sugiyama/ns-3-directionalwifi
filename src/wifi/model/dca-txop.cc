...
private:
  ...
  {
    m_txop->NotifyNextTransmission (manager);
  }
  DcaTxop *m_txop;
};
...
void
DcaTxop::NotifyNextTransmission (DcfManager* manager)
{
  NS_LOG_FUNCTION (this);
  if (m_currentPacket == 0)
    {
  ...
  // backoff init
  m_dcf->SetBackoff (m_dcf->GetFirstBackoffStart (), m_rng->GetNext (0, m_dcf->GetCw()));

  if (m_currentHdr.GetAddr1 ().IsGroup ())
    {
      manager->SetNextAntennaMode (WifiSwitchedBeamAntennaModel::OMNI);
    }
  else
    {
      WifiMacHeader hdr;
      ...
      if (NeedRts (fragment, &hdr)){
        ...
        if (m_currentHdr.GetAddr1 ().IsBroadcast ()){
          ...
          int nextAntennaMode = m_low->GetNextAntennaMode (m_currentHdr.GetAddr1 ());
          manager->SetNextAntennaMode (nextAntennaMode);
        }
      }else{
        // DATA
        if (m_currentHdr.GetAddr1 (). IsBroadcast ()){
  ...
  if (!NeedRtsRetransmission ())
    {
      ...
      m_currentPacket = 0;
      m_dcf->ResetCw ();
      m_low->SetAntennaMode (WifiSwitchedBeamAntennaModel::OMNI);
    }
  else
    {
