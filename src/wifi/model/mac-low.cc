...
namespace ns3 {
class AntennaListenerMacLow : public ns3::WifiAntennaListener
{
public:
  /**
   * Create a AntennaListenerMacLow for the given MacLow.
   *
   * \param state
   */
  AntennaListenerMacLow (ns3::MacLow *low)
    : m_low (low)
  {
  }
  virtual ~AntennaListenerMacLow ()
  {
  }
  virtual void NotifyChangeAntennaMode (int mode)
  {
    m_low->NotifyChangeAntennaModeNow (mode);
  }
private:
  ns3::MacLow *m_low;  //!< MacLow to forward events to
};
    ...
    m_ctsToSelfSupported (false)
{
  ...
  for(int i = 0; i < WifiAntennaModel::NUMBER_OF_ANTENNA_MODES; i++){
    m_lastNavDuration[i] = Seconds (0);
    m_lastNavStart[i] = Seconds (0);
  }
  m_promisc = false;
}
...
MacLow::SetPhy (Ptr<WifiPhy> phy)
{
  ...
  SetupAntennaListener (phy->GetAntenna ());
}
void
...
MacLow::NotifySwitchingStartNow (Time duration)
{
  ...
  m_lastNavStart[m_antennaMode] = Simulator::Now ();
  m_lastNavDuration[m_antennaMode] = Seconds (0);
  m_currentPacket = 0;
  m_listener = 0;
}
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiMode txMode, WifiPreamble preamble)
{
  ...
  int nextAntennaMode = WifiSwitchedBeamAntennaModel::OMNI;
  if (flgGeoTag){
    Ptr<MobilityModel> mobility = m_phy->GetMobility ()->GetObject<MobilityModel> ();
    Vector v = mobility->GetPosition ();
    Angles bet = Angles (geoTag.Get (), v);
    nextAntennaMode = GetNextAntennaMode (bet);
  }

  bool isPrevNavZero = IsNavZero (nextAntennaMode);
  NS_LOG_DEBUG ("duration/id=" << hdr.GetDuration ());
  NotifyNav (packet,hdr, txMode, preamble, nextAntennaMode);
  if (hdr.IsRts ())
    {
...
void
MacLow::NotifyNav (Ptr<const Packet> packet,const WifiMacHeader &hdr, WifiMode txMode, WifiPreamble preamble, int mode)
{
  NS_LOG_FUNCTION (this << mode);
  NS_ASSERT (m_lastNavStart[mode] <= Simulator::Now ());
  ...
  /// but we don't for now because we do not generate them.
  else if (hdr.GetAddr1 () != m_self)
    {
      ...
      bool navUpdated = DoNavStartNow (duration, mode);
...
MacLow::DoNavResetNow (Time duration)
{
  ...
  m_lastNavStart[m_antennaMode] = Simulator::Now ();
  m_lastNavStart[m_antennaMode] = duration;
}
bool
MacLow::DoNavStartNow (Time duration, int mode)
{
  NS_LOG_FUNCTION (this << duration << mode);
  for (DcfListenersCI i = m_dcfListeners.begin (); i != m_dcfListeners.end (); i++)
    {
      (*i)->NavStart (duration, mode);
    }
  Time newNavEnd = Simulator::Now () + duration;
  Time oldNavEnd = m_lastNavStart[WifiSwitchedBeamAntennaModel::OMNI] +
    m_lastNavDuration[WifiSwitchedBeamAntennaModel::OMNI];
  
  if (newNavEnd > oldNavEnd)
    {
      m_lastNavStart[WifiSwitchedBeamAntennaModel::OMNI] = Simulator::Now ();
      m_lastNavDuration[WifiSwitchedBeamAntennaModel::OMNI] = duration;
      NS_LOG_DEBUG ("nav end time:[OMNI]:" << m_lastNavStart[WifiSwitchedBeamAntennaModel::OMNI] + m_lastNavDuration[WifiSwitchedBeamAntennaModel::OMNI]);
    }

  newNavEnd = Simulator::Now () + duration;
  oldNavEnd = m_lastNavStart[mode] + m_lastNavDuration[mode];
  if (newNavEnd > oldNavEnd)
    {
      m_lastNavStart[mode] = Simulator::Now ();
      m_lastNavDuration[mode] = duration;
      NS_LOG_DEBUG ("nav end time:[mode]:" << m_lastNavStart[mode] + m_lastNavDuration[mode]);
      return true;
    }
  return false;
}
...
MacLow::SendRtsForPacket (void)
{
  ...
  // set antenna mode
  if(rts.GetAddr1().IsBroadcast()){
    SetAntennaMode (WifiSwitchedBeamAntennaModel::OMNI);
  }else{
    bool existsAddress = false;
    Ptr<MobilityModel> mobility = m_phy->GetMobility ()->GetObject<MobilityModel> ();
    Vector v = mobility->GetPosition ();
    Angles bet = m_phy->GetGeographyTable ()->GetAngle (rts.GetAddr1(), v, &existsAddress);
    if(existsAddress){
      SetAntennaMode (bet);
    }else{
      SetAntennaMode (WifiSwitchedBeamAntennaModel::OMNI);
...
MacLow::SendDataPacket (void)
{
  ...
  // set antenna mode
  NS_LOG_DEBUG (m_currentHdr.GetAddr1 ());
  if(m_currentHdr.GetAddr1 ().IsBroadcast ()){
    SetAntennaMode (WifiSwitchedBeamAntennaModel::OMNI);
  }
...
bool
MacLow::IsNavZero (int mode) const
{
  if (m_lastNavStart[mode] + m_lastNavDuration[mode] < Simulator::Now ())
    {
      return true;
    }
  else
...
MacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
{
  ...
  m_sendAckEndEvent = Simulator::Schedule (MacLow::GetAckDuration (ackTxVector), &MacLow::SendAckEnd, this);

  ForwardDown (packet, &ack, ackTxVector, preamble);
}

void
MacLow::SendAckEnd (void)
{
  NS_LOG_FUNCTION (this);
  SetAntennaMode (WifiSwitchedBeamAntennaModel::OMNI);
}
...
void
MacLow::SetupAntennaListener (Ptr<WifiAntennaModel> antenna)
{
  ...
  m_antennaListener = new AntennaListenerMacLow (this);
  antenna->RegisterListener (m_antennaListener);
}
void
MacLow::NotifyChangeAntennaModeNow (int mode)
{
  //  NS_LOG_FUNCTION (this << mode);
  m_antennaMode = mode;
}
...
int
MacLow::GetNextAntennaMode (Mac48Address addr)
{
  NS_LOG_FUNCTION (this);
  ...
  Ptr<MobilityModel> mobility = m_phy->GetMobility ()->GetObject<MobilityModel> ();
  Vector v = mobility->GetPosition ();
  Angles bet = m_phy->GetGeographyTable ()->GetAngle (addr, v, &existsAddress);
  if(existsAddress){
    return m_phy->GetAntenna ()->GetNextAntennaMode (bet);
  }else{
    return WifiSwitchedBeamAntennaModel::OMNI;
  }
}
int
MacLow::GetNextAntennaMode (Angles bet)
{
  NS_LOG_FUNCTION (this);
  return m_phy->GetAntenna ()->GetNextAntennaMode (bet);
}
